...
def main():
  ...
  # Read the matches
  matches = {} #So the matches between pairs. For all the pairs of images
  for image_pair in itertools.combinations(image_names, 2):
    matches[image_pair] = ReadFeatureMatches(image_pair, data_folder)

  K = ReadKMatrix(data_folder) # The matrix which rapresents the camera paramters
  ...
  e_matches = GetPairMatches(e_im1_name, e_im2_name, matches) #Get the matches between e_im1 and e_im2 in the matches collection matches.
  ...
  e_im1.SetPose(np.eye(3), np.asarray([0,0,0]))
  num_points = []
  for i in range(len(possible_relative_poses)):
    e_im2.SetPose(possible_relative_poses[i][0], possible_relative_poses[i][1])
    points3D_front, _, _ = TriangulatePoints(K, e_im1, e_im2, e_matches)
    num_points.append(len(points3D_front))
  index = np.argmax(num_points)
  e_im2.SetPose(possible_relative_poses[index][0], possible_relative_poses[index][1])
  #TODO: Check if the implementation of the pose selection is correct.
